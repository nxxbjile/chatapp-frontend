...
const ChatMessage = ({ sender, message, time}) => {
  ...
  return (
    <div className={` ${sender === user.username ? 'self-end' : 'self-start'} max-w-[200px] w-3/4 h-fit my-1 `}>
        <div className={`w-full h-fit rounded-md ${sender === user.username ? 'bg-rose-500' : 'bg-neutral-800'}  p-2 flex flex-col`}>
            ...
            <div dangerouslySetInnerHTML={{ __html:styleTextWithRegex(message)  }} className={`w-full break-all h-fit ${sender === user.username ? 'text-white' : 'text-neutral-400'} py-2`} />
            {
